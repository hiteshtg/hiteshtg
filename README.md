# 👋 Hi, I'm Hitesh Sharma

🎓 BCA Student | 💻 Aspiring Cloud & Security Engineer | 🛠️ Project-Driven Learner  

I focus on **hands-on projects** in cloud, Linux, networking.  
Currently building real-world projects instead of collecting certificates.

---

## 🧠 What I’m Working On
- Cloud projects on **AWS (Free Tier / Student)**
- Linux-based security & monitoring setups
- Automation using **Python**
- Learning system-level concepts (Networking, OS, Security)

---

## 🛠️ Tech Stack

### 💻 Languages
![Python](https://img.shields.io/badge/Python-000000?style=for-the-badge&logo=python)
![Bash](https://img.shields.io/badge/Bash-000000?style=for-the-badge&logo=gnu-bash)

### ☁️ Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-000000?style=for-the-badge&logo=amazonaws)
![Docker](https://img.shields.io/badge/Docker-000000?style=for-the-badge&logo=docker)

### 🔐 Security & Networking
![Linux](https://img.shields.io/badge/Linux-000000?style=for-the-badge&logo=linux)
![Zeek](https://img.shields.io/badge/Zeek-000000?style=for-the-badge)
![Suricata](https://img.shields.io/badge/Suricata-000000?style=for-the-badge)

### ⚙️ Tools
![Git](https://img.shields.io/badge/Git-000000?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-000000?style=for-the-badge&logo=github)
![Emacs](https://img.shields.io/badge/VS_Code-000000?style=for-the-badge&logo=visualstudiocode)

---
<!--
## 🚀 Featured Projects

### 🔹 NETMON – Network Monitoring & Alerting System
- IDS-based alert ingestion using **Suricata**
- Ticketing system with SQLite
- Designed for SOC-style workflows  
➡️ *Repo:* `<link>`

### 🔹 SOAR Mini Project
- Automated incident response workflow
- Log analysis + alert handling
➡️ *Repo:* `<link>`

### 🔹 Cloud Infrastructure Lab
- VPC, EC2, IAM, logging & monitoring
- Security-focused configurations
➡️ *Repo:* `<link>`

--->

> I believe skills are proven through **projects**.
